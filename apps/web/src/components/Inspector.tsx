...
export function Inspector({
    ...
    return (
      <div className="inspector">
        <div className="inspector-empty">
          ...
          <h3>选择一项创作素材</h3>
          <p>生成任务会按文生图、图生图、文生视频和图生视频显示不同参数。</p>
  ...
  const needsInput = mode.inputKind !== 'none';
  const requiredInputs = mode.inputKind === 'two-images' ? 2 : needsInput ? 1 : 0;
  ...
  return (
    <div className="inspector">
      ...
      {isGeneration && (
        <>
          <div className="inspector-section-title">
            <SlidersHorizontal size={14} /> 生成模式
          </div>
          <SelectField
            label="模式"
            value={mode.id}
          ...
          <label>
            模型 ID
            <input
              value={String(data.model ?? (mode.mediaType === 'video' ? 'Video Provider' : 'Image Provider'))}
              onChange={(event) => onUpdate({ model: event.target.value })}
              placeholder="由 Provider 支持的真实模型名称"
            />
          </label>
          ...
          {mode.mediaType === 'image' && (
            <>
              ...
              {(operation === 'image-to-image' || operation === 'inpaint' || operation === 'outpaint') && (
                <>
                  <NumberField
      ...
      {isMediaOutput && (
        <button className="generate-button" onClick={onGenerate}>
          ...
          <span>{type === 'videoOutput' ? '基于此视频继续生成' : '基于此图片创建新任务'}</span>
        </button>
      )}
      ...
      <div className="security-note">
        ...
        <span>任务会保存操作类型、输入素材、提示词、模型参数和输出版本。</span>
      </div>
    </div>
  );
}
